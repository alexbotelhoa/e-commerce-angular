...
export const studentReportController = (env: Environment, db: DatabaseService, readonlyDb: DatabaseService, redis?: Redis) => async (request: FastifyRequest, reply: FastifyReply) => {
    if (redis) {
        const response = await redis.get("studentReport")
    ...
    ORDER BY user.id
        `)
        if (result.length === 0) {
            await redis.del("studentReport")
            reply.header("Content-Type", 'text/csv')
            reply.send(parse(result))
        } else {
            // reply.header("Content-Type", 'text/csv')
            await redis.set("studentReport", JSON.stringify(result), 'ex', 43200)
        }
    }
}