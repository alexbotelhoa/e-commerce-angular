...
export const studentInterestReportController = (env: Environment, db: DatabaseService, readonlyDb: DatabaseService, redis?: Redis) => async (req: FastifyRequest, reply: FastifyReply) => {
	if (redis) {
		const response = await redis.get("studentInterest")
	...
	left JOIN (
		...
		if (result.length === 0) {
			await redis.del("studentInterest")
			reply.header("Content-Type", 'text/csv')
			reply.send(parse(result))
		} else {
			// reply.header("Content-Type", 'text/csv')
			await redis.set("studentInterest", JSON.stringify(result), 'ex', 43200)
		}
	}
}