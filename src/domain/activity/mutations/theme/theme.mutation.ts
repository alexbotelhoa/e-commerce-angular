...
export const activateThemeMutationResolver: GQLMutationResolvers['activateTheme'] = async (obj, { id }, context) => {
    return await getThemeById(context.database)(await updateTheme(context.database)({ active: true })(builder => builder.andWhere('id', id)));
}

export const deactivateThemeMutationResolver: GQLMutationResolvers['deactivateTheme'] = async (obj, { id }, context) => {
    return await getThemeById(context.database)(await updateTheme(context.database)({ active: false })(builder => builder.andWhere('id', id)));
}