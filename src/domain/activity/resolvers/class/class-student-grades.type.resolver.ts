    ...
    batchFn: (db, params) => async (ids) => {
        ...
        const result = await db.raw<[ClassStudentGrade[]]>(`
            ...
            LEFT JOIN (
                ...
                GROUP BY classId
            ) AS totalActivitiesByClass ON totalActivitiesByClass.classId = enrollment_class.classId
            LEFT JOIN (
                ...
                WHERE class.id IN (${idsParameters})
                GROUP BY classId
            ) AS totalProgressChecksByClass ON totalProgressChecksByClass.classId = enrollment_class.classId
            LEFT JOIN (
                SELECT count(*) AS completedActivities, activity_timer.userId, activity_timer.classId AS classId
                FROM activity_timer
            ...
            ) AS completedActivitiesByUserAndClass ON 
                completedActivitiesByUserAndClass.classId = enrollment_class.classId
                AND completedActivitiesByUserAndClass.userId = user.id
            LEFT JOIN (
                SELECT count(*) AS completedProgressChecks, activity_timer.userId, activity_timer.classId AS classId
                FROM activity_timer
            ...
            ) AS completedProgressChecksByUserAndClass ON 
