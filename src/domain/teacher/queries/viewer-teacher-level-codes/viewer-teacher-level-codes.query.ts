...
export const viewerTeacherLevelCodesQueryResolver: GQLQueryResolvers['viewerTeacherLevelCodes'] = async (obj, params, context) => {
    ...
    if (!params.filters?.userId) {
        query.innerJoin(TEACHER_CLASS_TABLE, `${TEACHER_CLASS_TABLE}.classId`, `${CLASS_TABLE}.id`)
            .andWhere('teacherId', user?.id);
    } else {
        query.innerJoin(ENROLLMENT_TABLE, `${LEVEL_CODE_TABLE}.id`, `${ENROLLMENT_TABLE}.levelCodeId`)
            .andWhere(`${ENROLLMENT_TABLE}.userId`, params.filters.userId);
    }
